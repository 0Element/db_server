...
Influx::Influx(const char* addr, int port, int count_msgs, int count_thrs)
{
    ...
    GenMessagesShort(count_msgs, count_thrs);
}
...
void Influx::GenMessagesShort(int count_msgs, int count_thrs)
{
    std::string src_msg("cpu_load_short value=\"value");

    for (int i = 0, j = 1; i < count_msgs; i++) {
        std::string msg = src_msg + std::to_string(i) + "\"";;
...
void Influx::SendTo()
{
    for (int i = 0; i < count_thrs; i++) {
        ...
        std::thread thr(&Influx::_SendTo, this, i);
        thr.detach();
        vec_thrs.push_back(move(thr));
        //v_socks_ret_t v_s_rets = v_send_msg(vec_socks, msg.c_str(), msg.size());
    }
...
void Influx::_SendTo(int i)
{
    ...
    while(running) {
        ...
        send_to(sock, msg.c_str(), msg.size(), srv);
